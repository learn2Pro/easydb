...
import org.gibello.zql.javacc.ZqlJJParser;
...
public class ZqlParser {
    ...
    private static ZqlJJParser _parser;

    static {
        //singleton init
        InputStream stream = new ByteArrayInputStream("1=1".getBytes
                (StandardCharsets.UTF_8));
        _parser = new ZqlJJParser(stream);
    }

    /**
     * Test program: Parses SQL statements from stdin or from a text file.<br> If the program receives one argument, it
     * is an SQL text file name; if there's no argument, the program reads from stdin.
     */
    ...
    public ZqlParser(String sql) {
        initParser(new ByteArrayInputStream(sql.getBytes(StandardCharsets.UTF_8)));
    }
    ...
    public void addCustomFunction(String fct, int nparm) {
        org.gibello.zql.ZUtils.addCustomFunction(fct, nparm);
    }
     ...
     */
    public org.gibello.zql.ZExp readExpression() throws ParseException {
        if (_parser == null) {
            throw new ParseException(
                    "Parser not initialized: use initParser(InputStream);");
        }
