...
public class TableStatsTest extends SimpleDbTestBase {
    ...
    private double[] getRandomTableScanCosts(int[] pageNums, int[] ioCosts)
    ...
    /**
     * Verify the cost estimates of scanning various numbers of pages from a HeapFile This test checks that the
     * estimateScanCost is: +linear in numPages when IO_COST is constant +linear in IO_COST when numPages is constant
    ...
    /**
     * Verify that selectivity estimates do something reasonable. Don't bother splitting this into N different functions
     * for each possible Op because we will probably catch any bugs here in IntHistogramTest, so we hopefully don't need
     * all the JUnit checkboxes.
     */
    @Test
