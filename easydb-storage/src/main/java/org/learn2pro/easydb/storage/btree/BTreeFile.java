...
import org.learn2pro.easydb.storage.AbstractDbFileIterator;
...
import org.learn2pro.easydb.storage.btree.BTreeFileEncoder.EntryComparator;
import org.learn2pro.easydb.storage.btree.BTreeFileEncoder.TupleComparator;
import org.learn2pro.easydb.storage.common.Field;

/**
 * BTreeFile is an implementation of a DbFile that stores a B+ tree. Specifically, it stores a pointer to a root page, a
 * set of internal pages, and a set of leaf pages, which contain a collection of tuples in sorted order. BTreeFile works
 * closely with BTreeLeafPage, BTreeInternalPage, and BTreeRootPtrPage. The format of these pages is described in their
 * constructors.
 *
 * @author Becca Taft
...
public class BTreeFile implements DbFile {
    ...
    /**
     * Returns an ID uniquely identifying this BTreeFile. Implementation note: you will need to generate this tableid
     * somewhere and ensure that each BTreeFile has a "unique id," and that you always return the same value for a
     * particular BTreeFile. We suggest hashing the absolute file name of the file underlying the BTreeFile, i.e.
    ...
    /**
     * Read a page from the file on disk. This should not be called directly but should be called from the BufferPool
     * via getPage()
     *
    ...
    /**
     * Write a page to disk.  This should not be called directly but should be called from the BufferPool when pages are
     * flushed to disk
     *
    ...
    /**
     * Recursive function which finds and locks the leaf page in the B+ tree corresponding to the left-most page
     * possibly containing the key field f. It locks all internal nodes along the path to the leaf node with READ_ONLY
     * permission, and locks the leaf node with permission perm.
     *
...
//        // some code goes here
        ...
        if (!(page instanceof BTreeInternalPage)) {
            throw new DbException(
                    String.format("the page must be internal page,actually is:%s!", page.getClass().getSimpleName()));
        }
        ...
        if (entry == null) {
            throw new DbException("not found the matching search entry or the internal page is empty!");
        }
        return findLeafPage(tid, dirtypages, entry.getRightChild(), perm, f);
    }

    /**
     * Convenience method to find a leaf page when there is no dirtypages HashMap. Used by the BTreeFile iterator.
     *
     * @param tid - the transaction id
     * @param pid - the current page being searched
     * @param perm - the permissions with which to lock the leaf page
    ...
    /**
     * Split a leaf page to make room for new tuples and recursively split the parent node as needed to accommodate a
     * new entry. The new entry should have a key matching the key field of the first tuple in the right-hand page (the
     * key is "copied up"), and child pointers pointing to the two leaf pages resulting from the split.  Update sibling
     * pointers and parent pointers as needed.
     *
     ...
     */
    public BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
            Field field)
        ...
        List<Tuple> afterHalf;
        List<Tuple> all = Lists.newArrayList();
        Iterator<Tuple> iterator = page.iterator();
        while (iterator.hasNext()) {
            Tuple current = iterator.next();
            all.add(current);
        }
        Collections.sort(all, new TupleComparator(keyField));
        Tuple mid = all.get(all.size() / 2);
        boolean isLeft = field.compare(Op.LESS_THAN, mid.getField(keyField));
//        int index = findInsertIndexOfLeafPage(field, all);
        //split the page
        afterHalf = all.subList(all.size() / 2, all.size());
        for (Tuple entry : afterHalf) {
            page.deleteTuple(entry);
            right.insertTuple(entry.clone());
        }
        BTreeEntry pushUpEntry = new BTreeEntry(mid.getField(keyField), page.getId(), right.getId());
        //push up
        ...
        updateSibling(page, right, tid, dirtypages);
        return isLeft ? page : right;

        /**
         * solution 2.
         */
        //1. create split right page
//        BTreeLeafPage right = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
...
//            right.insertTuple(current);
//        }
//        Field rightFirst = right.reverseIterator().next().getField(keyField);
//        BTreeEntry pushUpEntry = new BTreeEntry(rightFirst, page.getId(), right.getId());
//        //push up
//        BTreeInternalPage parent = pushUpEntry(tid, dirtypages, pushUpEntry, right.getId(), page.getParentId(),
//                page.getId());
//        updateParentPointers(tid, dirtypages, parent);
//        updateSibling(page, right, tid, dirtypages);
//        return field.compare(Op.GREATER_THAN_OR_EQ, rightFirst) ? right : page;

    }

    private void updateSibling(BTreeLeafPage left, BTreeLeafPage right, TransactionId tid,
            HashMap<PageId, Page> dirtypages) throws DbException, TransactionAbortedException {
        // left<->old_right
        // left<->right<->old_right
        if (left.getRightSiblingId() != null) {
            //1. right<-old_right
            BTreeLeafPage oldRightSibling = (BTreeLeafPage) getPage(tid, dirtypages, left.getRightSiblingId(),
                    Permissions.READ_WRITE);
            oldRightSibling.setLeftSiblingId(right.getId());
        }
        //2. left<->right->old_right
        right.setRightSiblingId(left.getRightSiblingId());
        right.setLeftSiblingId(left.getId());
        left.setRightSiblingId(right.getId());
    }

    /**
     * Split an internal page to make room for new entries and recursively split its parent page as needed to
     * accommodate a new entry. The new entry for the parent should have a key matching the middle key in the original
     * internal page being split (this key is "pushed up" to the parent). The child pointers of the new parent entry
     * should point to the two internal pages resulting from the split. Update parent pointers as needed.
     *
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        List<BTreeEntry> afterHalf;
        List<BTreeEntry> all = Lists.newArrayList();
        Iterator<BTreeEntry> iterator = page.iterator();
        while (iterator.hasNext()) {
            BTreeEntry current = iterator.next();
            all.add(current);
        }
        Collections.sort(all, new EntryComparator());
        BTreeEntry mid = all.get(all.size() / 2 + 1);
        Boolean isLeft = field.compare(Op.LESS_THAN, mid.getKey());
        //split the page
        BTreeEntry pushUpEntry = new BTreeEntry(all.get(all.size() / 2).getKey(), page.getId(), right.getId());
        afterHalf = all.subList(all.size() / 2 + 1, all.size());
        for (BTreeEntry entry : afterHalf) {
            page.deleteKeyAndRightChild(entry);
            right.insertEntry(entry.clone());
        }
        page.deleteKeyAndRightChild(all.get(all.size() / 2));
        //push up
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), pushUpEntry.getKey());
        parent.insertEntry(pushUpEntry);
        updateParentPointers(tid, dirtypages, parent);
...
//        for (int i = 0; i < total / 2; i++) {
//            BTreeEntry current = iterator.next();
//            page.deleteKeyAndRightChild(current);
//            right.insertEntry(current);
//        }
//        BTreeEntry leftMost = iterator.next();
//        Field f = leftMost.getKey();
//        page.deleteKeyAndRightChild(leftMost);
//
//        BTreeEntry pushUpEntry = new BTreeEntry(f, page.getId(), right.getId());
//        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), f);
//        parent.insertEntry(pushUpEntry);
//        updateParentPointers(tid, dirtypages, parent);
//        updateParentPointers(tid, dirtypages, right);
//        return field.compare(Op.GREATER_THAN_OR_EQ, f) ? right : page;
    ...
    /**
     * Method to encapsulate the process of getting a parent page ready to accept new entries. This may mean creating a
     * page to become the new root of the tree, splitting the existing parent page if there are no empty slots, or
     * simply locking and returning the existing parent page.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     * @param parentId - the id of the parent. May be an internal page or the RootPtr page
     * @param field - the key of the entry which will be inserted. Needed in case the parent must be split to
     ...
     */
    private void updateParentPointers(
    ...
    /**
     * Method to encapsulate the process of locking/fetching a page.  First the method checks the local cache
     * ("dirtypages"), and if it can't find the requested page there, it fetches it from the buffer pool. It also adds
     * pages to the dirtypages cache if they are fetched with read-write permission, since presumably they will soon be
    ...
    /**
     * Insert a tuple into this BTreeFile, keeping the tuples in sorted order. May cause pages to split if the page
     * where tuple t belongs is full.
     *
     * @param tid - the transaction id
     * @param t - the tuple to insert
     * @return a list of all pages that were dirtied by this operation. Could include many pages since parent pointers
     *         will need to be updated when an internal node splits.
    ...
    /**
     * Handle the case when a B+ tree page becomes less than half full due to deletions. If one of its siblings has
     * extra tuples/entries, redistribute those tuples/entries. Otherwise merge with one of the siblings. Update
     * pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     ...
     * @throws IOException
     * @throws TransactionAbortedException
     * @see #handleMinOccupancyLeafPage(TransactionId, HashMap, BTreeLeafPage, BTreeInternalPage, BTreeEntry,
     *         BTreeEntry)
     * @see #handleMinOccupancyInternalPage(TransactionId, HashMap, BTreeInternalPage, BTreeInternalPage, BTreeEntry,
     *         BTreeEntry)
     */
    ...
    /**
     * Handle the case when a leaf page becomes less than half full due to deletions. If one of its siblings has extra
     * tuples, redistribute those tuples. Otherwise merge with one of the siblings. Update pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     ...
     */
    private void handleMinOccupancyLeafPage(
            TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
    ...
    /**
     * Handle the case when an internal page becomes less than half full due to deletions. If one of its siblings has
     * extra entries, redistribute those entries. Otherwise merge with one of the siblings. Update pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     ...
     * @throws IOException
     * @throws TransactionAbortedException
     * @see #mergeInternalPages(TransactionId, HashMap, BTreeInternalPage, BTreeInternalPage, BTreeInternalPage,
     *         BTreeEntry)
     * @see #stealFromLeftInternalPage(TransactionId, HashMap, BTreeInternalPage, BTreeInternalPage,
     *         BTreeInternalPage, BTreeEntry)
     * @see #stealFromRightInternalPage(TransactionId, HashMap, BTreeInternalPage, BTreeInternalPage,
     *         BTreeInternalPage, BTreeEntry)
     */
    ...
    /**
     * Steal tuples from a sibling and copy them to the given page so that both pages are at least half full.  Update
     * the parent's entry so that the key matches the key field of the first tuple in the right-hand page.
     *
     * @param page - the leaf page which is less than half full
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int less = page.getNumTuples();
        int more = sibling.getNumTuples();
        int half = (less + more) / 2;
        Iterator<Tuple> iterator = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
        Tuple tp = null;
        while (less < half) {
            Tuple current = iterator.next();
            sibling.deleteTuple(current);
            page.insertTuple(current);
            tp = current;
            less++;
        }
        if (tp != null) {
            entry.setKey(tp.getField(keyField));
        }
     ...
     * Keys can be thought of as rotating through the parent entry, so the original key in the parent is "pulled down"
     * to the right-hand page, and the last key in the left-hand page is "pushed up" to the parent.  Update parent
     * pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
...
//        BTreeEntry center = new BTreeEntry(parentEntry.getKey(), move.getRightChild(), leftMostInRight);
...
//        updateParentPointers(tid, dirtypages, page);
        ...
        BTreeEntry move = moveEntry.next();
        BTreeEntry center = new BTreeEntry(parentEntry.getKey(), move.getRightChild(),
     ...
     * Keys can be thought of as rotating through the parent entry, so the original key in the parent is "pulled down"
     * to the left-hand page, and the last key in the right-hand page is "pushed up" to the parent.  Update parent
     * pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
...
//        BTreeEntry move = moveEntry.next();
//        BTreeEntry center = new BTreeEntry(parentEntry.getKey(),rightMostInLeft,move.getLeftChild());
//        page.insertEntry(center);
//        while (less < half-1) {
...
//        updateParentPointers(tid,dirtypages,page);
        ...
        //将parent的entry移动到page
        BTreeEntry move = moveEntry.next();
        BTreeEntry center = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(),
                move.getLeftChild());
        page.insertEntry(center);
    ...
    /**
     * Merge two leaf pages by moving all tuples from the right page to the left page. Delete the corresponding key and
     * right child pointer from the parent, and recursively handle the case when the parent gets below minimum
     * occupancy. Update sibling pointers as needed, and make the right page available for reuse.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    public void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // setEmptyPage() will be useful here
        Iterator<Tuple> iterator = rightPage.iterator();
        while (iterator.hasNext()) {
            Tuple current = iterator.next();
            rightPage.deleteTuple(current);
            leftPage.insertTuple(current);
        }
//        Iterator<BTreeEntry> entryIter = parent.iterator();
//        BTreeEntry nextEntry = null;
//        while (entryIter.hasNext()) {
//            if (entryIter.next().getKey().compare(Op.EQUALS, parentEntry.getKey())) {
//                break;
//            }
//        }

        //set next entry left child
//        if (entryIter.hasNext()) {
//            nextEntry = entryIter.next();
//            nextEntry.setLeftChild(leftPage.getId());
//        }
        //update sibling
        BTreePageId rightSibling = rightPage.getRightSiblingId();
        if (rightSibling != null) {
            //left->right<->right_right
            BTreeLeafPage rightSiblingPage = (BTreeLeafPage) getPage(tid, dirtypages, rightSibling,
                    Permissions.READ_WRITE);
            //left<-right_right
...
//        deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
    ...
    /**
     * Merge two internal pages by moving all entries from the right page to the left page and "pulling down" the
     * corresponding key from the parent entry. Delete the corresponding key and right child pointer from the parent,
     * and recursively handle the case when the parent gets below minimum occupancy. Update parent pointers as needed,
    ...
    public void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        BTreeEntry rightMostInLeft = leftIte.next();
        Iterator<BTreeEntry> rightIte = rightPage.iterator();
        BTreeEntry leftMostInRight = rightIte.next();
        //handling parent entry
        deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
        //update parent entry pointer
        parentEntry.setLeftChild(rightMostInLeft.getRightChild());
        parentEntry.setRightChild(leftMostInRight.getLeftChild());
        //insert parent entry & left most in right
        leftPage.insertEntry(parentEntry);
        rightPage.deleteKeyAndRightChild(leftMostInRight);
        leftPage.insertEntry(leftMostInRight);
        while (rightIte.hasNext()) {
            BTreeEntry current = rightIte.next();
            rightPage.deleteKeyAndRightChild(current);
            leftPage.insertEntry(current);
        }
        //set right page empty
        setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber());
        //update parent
     ...
     * If the parent becomes empty (no keys remaining), that indicates that it was the root node and should be replaced
     * by its one remaining child.  Otherwise, if it gets below minimum occupancy for non-root internal nodes, it should
     * steal from one of its siblings or merge with a sibling.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Delete a tuple from this BTreeFile. May cause pages to merge or redistribute entries/tuples if the pages become
     * less than half full.
     *
     * @param tid - the transaction id
     * @param t - the tuple to delete
     * @return a list of all pages that were dirtied by this operation. Could include many pages since parent pointers
     *         will need to be updated when an internal node merges.
     * @see #handleMinOccupancyPage(TransactionId, HashMap, BTreePage)
     */
    ...
    /**
     * Get a read lock on the root pointer page. Create the root pointer page and root page if necessary.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     ...
     * @throws TransactionAbortedException
     */
    BTreeRootPtrPage getRootPtrPage(
    ...
    /**
     * Get the page number of the first empty page in this BTreeFile. Creates a new page if none of the existing pages
     * are empty.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Method to encapsulate the process of creating a new page.  It reuses old pages if possible, and creates a new
     * page if none are available.  It wipes the page on disk and in the cache and returns a clean copy locked with
    ...
    /**
     * Mark a page in this BTreeFile as empty. Find the corresponding header page (create it if needed), and mark the
     * corresponding slot in the header page as empty.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
...
//		}
     ...
     * This method will acquire a read lock on the affected pages of the file, and may block until the lock can be
     * acquired.
     *
     * @param tid - the transaction id
    ...
    /**
     * Get an iterator for all tuples in this B+ tree file in sorted order. This method will acquire a read lock on the
     * affected pages of the file, and may block until the lock can be acquired.
     *
     * @param tid - the transaction id
...
class BTreeFileIterator extends AbstractDbFileIterator {
    ...
    /**
     * Read the next tuple either from the current page if it has more tuples or from the next page by following the
...
class BTreeSearchIterator extends AbstractDbFileIterator {
    ...
    /**
     * Read the next tuple either from the current page if it has more tuples matching the predicate or from the next
     * page by following the right sibling pointer.
     *
