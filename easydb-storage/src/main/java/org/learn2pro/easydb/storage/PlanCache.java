...
public class PlanCache {
    ...
    public Vector<LogicalJoinNode> getByFirstMatchLength(int len) {
        for (Entry<Set<LogicalJoinNode>, Vector<LogicalJoinNode>> entry : bestOrders.entrySet()) {
            if (entry.getKey().size() != len) {
                continue;
            }
            return entry.getValue();
        }
        throw new IllegalStateException("not found the matching length choice!");
    }
}
